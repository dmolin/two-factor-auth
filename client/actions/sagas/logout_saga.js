...
export default function* logoutWatcher() {
  yield* takeEvery(ACTION_LOGOUT, function* logout(action) {
    ...
    try {
      ...
      /*
       * ideally this should be changed for a purer approach: just dispatch the POPUP_MESSAGE_SHOW event with
       * msgType: 'error',
       * msg: error.message
...
Meteor.startup(() => {
  Tracker.autorun(() => {
    ...
    if(!Meteor.user() && store.getState().auth.state === LoginStates.LOGGED_IN) {
